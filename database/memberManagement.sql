...
CREATE DATABASE memberManagement CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
ALTER TABLE
  `xuly`
MODIFY
  COLUMN `MaXL` int(10) AUTO_INCREMENT;

ALTER TABLE
  `thietbi`
MODIFY
  COLUMN `MaTB` int(10) AUTO_INCREMENT;

ALTER TABLE
  `thongtinsd`
MODIFY
  COLUMN `MaTT` int(10) AUTO_INCREMENT;
...
ALTER TABLE
  `thongtinsd`
ADD
  CONSTRAINT `fk_thongtinsd_MaTV` FOREIGN KEY (`MaTV`) REFERENCES `thanhvien` (`MaTV`) ON DELETE CASCADE;

ALTER TABLE
  `thongtinsd`
ADD
  CONSTRAINT `fk_thongtinsd_MaTB` FOREIGN KEY (`MaTB`) REFERENCES `thietbi` (`MaTB`) ON DELETE CASCADE;

ALTER TABLE
  `xuly`
ADD
  CONSTRAINT `fk_xuly_MaTV` FOREIGN KEY (`MaTV`) REFERENCES `thanhvien` (`MaTV`) ON DELETE CASCADE;

CREATE DEFINER = `root` @`localhost` EVENT `DeleteExpiredBookings` ON SCHEDULE EVERY 1 MINUTE STARTS '2024-05-05 23:21:10' ON COMPLETION NOT PRESERVE ENABLE DO
DELETE FROM
  thongtinsd
WHERE
  TGMuon IS NULL
  AND tgdat_cho <= DATE_SUB(NOW(), INTERVAL 1 HOUR);