...
ALTER TABLE `xuly`
  ...
  FOREIGN KEY (`MaTV`) REFERENCES `thanhvien` (`MaTV`)
  ON DELETE CASCADE;
CREATE EVENT DeleteExpiredBookings
ON SCHEDULE EVERY 1 HOUR
DO
BEGIN
  DELETE FROM thongtinsd
  WHERE tgmuon IS NULL AND tgdatcho <= NOW() - INTERVAL 1 HOUR;
END
CREATE DEFINER=`root`@`localhost` EVENT `DeleteExpiredBookings` ON SCHEDULE EVERY 1 MINUTE STARTS '2024-05-05 23:21:10' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM thongtinsd
  WHERE TGMuon IS NULL AND tgdat_cho <= DATE_SUB(NOW(), INTERVAL 1 HOUR)
